...
Download `bands123` [(.zip)](#) and extract the compressed zip file to your computer.
...
_Note: some icons may look different, depending on the version of QGIS.__
...
## Image Exploration
In the Manage Layers Toolbar (on the far left), click the icon to Add Raster Layer, ![](images/mActionAddRasterLayer.png){#id .class height=20 width=20}, and add `band1.tif`.

The layer `band1` is added to the Layers Panel.

Double-click the layer name to open the Layer Properties window.

Under "General," find the layer's coordinate reference system.

> Q1. What is the layer's coordinate system?
<!-- North American Datum 1927, UTM Zone 11N -->

Under "Metadata," investigate the properties (you may have to scroll down to find Properties).

> Q2. What is the pixel size? What are the units for the pixel size?
<!-- 15 meters x 15 meters -->
>
> Q3. What is the data type?
<!-- byte -->
>
> Q4. In what wavelength of the electromagnetic (EM) spectrum is this image taken? What does this wavelength represent?
<!-- 0.556 visible green -->

Using WordPad, open the `band1.tif.aux.xml` file.
...
<!-- lines: 1700; samples: 1700
    1700 x 1700 x 1 byte = 2.89 MB -->
>
...
<!-- 1700 pixels x 15 m/px = 25.5 km -->

## Grayscale Image Analysis
Use the Pan Map and Zoom In/Out tools to investigate the grayscale band1 image.

> Q7. What is the large dark area in the right part of the image?
<!-- Lake Mead -->
>
> Q8. Can you find the location of one of the golf courses? *Hint: what would you expect the DN value of this feature to be in this wavelength?*
...
Find the southern-most boat marina.

> Q9. What could that bright group of pixels be?

Open the layer properties again.

Under "Histogram," click the Compute Histogram button, then click the "Save plot" button and save a copy of the histogram (e.g., `hist1.png`) to your workspace folder.
Investigate the histogram.

> Q10. What feature is likely causing the spike in the histogram at lower DN values?
<!-- Likely something metal; a boat or roof of a shed -->

Under "Style" in the Band rendering section, the color gradient is currently set to "Black to white" with a linear stretch between the defined min and max values.
Based on the histogram, see if you can highlight the lake and any patterns within it by applying different max/min values.

> Q11. What DN range did you choose? What patterns did this range reveal in the lake? What are the patterns, their general direction and what might their cause be?

Load back the image's original min and max values.

## Color Image Analysis
Click the Add Raster Layer button and add `band2.tif` and `band3.tif`.

Take a moment to investigate these two bands and how they compare to band 1.

Click on Raster in the Menu Bar, go to Miscellaneous, and click on "Merge."

- Click the "Select..." button next to "Input files"
    - use the Ctrl key to select "band1.tif," "band2.tif," and "band3.tif"
- Click on "Select..." next to Output file
    - in your workspace folder, save the output as "band321.tif"
- Check the box next to "No data value" and assign it to zero
- Check the box next to "Place each input file into a separate band"
- Click OK to run the tool

Uncheck band1, band2 and band3 layers.
...
- Load the min/max values
    - click load (use actual values instead of estimates)
- Band rendering
    - change contrast enhancement to "Stretch to MinMax"
...
Create a new Text Document in the folder on your USB drive.
...
In the Style of the layer properties:
...
### Directional filter
To highlight the patterns in the lake, we can run a directional filter over band1 that emphasizes brightness variations of interest.
...
Use the Print Composer button (![](images/mActionNewComposer.png){#id .class height=20 width=20}) to create output figures (also available through the Project menu in the menu bar).

You may give your new Print Composer a name (optional) and click OK.

The print composer window looks similar to QGIS, where there is a menu bar, toolbars, and a composer viewer, which shows you what your figure will look like (similar to the layouts in ArcGIS).

In the toolbar on the left, click Add New Map, ![](images/mActionAddMap.png){#id .class height=20 width=20}, then click and drag a square over your blank page.

Use the tools in the toolbar on the left to add other items to your figure, such as text (Add new label, ![](images/mActionLabel.png){#id .class height=20 width=20}), legend (Add new legend, ![](images/mActionAddLegend.png){#id .class height=20 width=20}), scale (Add new scalebar, ![](images/mActionScaleBar.png){#id .class height=20 width=20}).

For each item, when they are selected (using the Select/Move item tool, ![](images/mActionSelect.png){#id .class height=20 width=20}, in the toolbar on the left), their properties will appear in the panel on the right.
You can edit item properties in the Item properties tab (e.g., text and font styles for labels).

When the figure looks as you want it, click on either the Export as image (![](images/mActionSaveMapAsImage.png){#id .class height=20 width=20}) or Export as PDF (![](images/mActionSaveAsPDF.png){#id .class height=20 width=20}) button in the toolbar at the top (also found in the Composer menu in the menu bar).
